...
import { getCapabilities } from "./three/capabilities";
 ...
 *
 * A4 moves the pieces into the shared canvas, where they can pass each other
 * in depth. This file keeps the DOM version regardless, for three cases that
 * have no canvas at all — reduced motion, mobile, and no WebGL — and because
 * it is what carries the section's content to a screen reader. When the canvas
 * takes over, the DOM pieces stay in place and in the accessibility tree at
 * zero opacity, and the plates are drawn behind them.
 */
...
/** The drawn version of each piece. Positions and copy live in brandPlates. */
const PIECE_ART: Record<BrandPlate["id"], React.ReactNode> = {
  mark: (
...
export function ExplodedView() {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add(
        ...
        () => {
          ...
          const onCanvas = getCapabilities().webgl;
          ...
          if (onCanvas) {
            // The plates are drawn in the canvas. The DOM pieces stay laid out
            // and readable to assistive technology, and stay out of the way.
            ...
            const trigger = ScrollTrigger.create({
              ...
              onUpdate: (self) => publishProgress(PROGRESS_ID, self.progress),
              onRefresh: (self) => publishProgress(PROGRESS_ID, self.progress),
            });
          ...
          timeline.to(
            pieces,
            {
              x: (index: number) => BRAND_PLATES[index].explode.x * box().width,
              y: (index: number) => BRAND_PLATES[index].explode.y * box().height,
              rotate: (index: number) => BRAND_PLATES[index].explode.rotate,
              scale: (index: number) => BRAND_PLATES[index].explode.scale,
              ease: "power2.inOut",
              duration: 1,
            },
            0
          ...
          timeline.to(
            ...
            {
              x: (index: number) =>
                (BRAND_PLATES[index].cell.column - 1) *
                COLUMN_SPREAD *
                box().width,
              y: (index: number) =>
                (BRAND_PLATES[index].cell.row - 0.5) *
                2 *
                ROW_SPREAD *
                box().height,
              rotate: 0,
      ...
      // Mobile: no pin, no scroll capture, and no canvas plates. The pieces
      // simply arrive in order.
      mm.add(
  ...
  return (
    ...
    >
      <div
        // Pinned, this section starts at the top of the viewport, where the
        // fixed navbar was cutting the heading in half. The padding is what
        // clears it, and it gives the bottom row of the locked grid somewhere
        // to put its captions.
        className={`flex flex-col justify-center px-6 md:px-12 max-w-[1700px] mx-auto py-24 ${
          prefersReducedMotion ? "" : "md:h-screen md:pt-28 md:pb-10"
        }`}
      >
        ...
        */}
        ...
        >
          {/* Hands the stage box to the canvas: the plates are drawn where
              the layout says this section is, not at a guessed position. */}
          {!prefersReducedMotion && (
            <SceneAnchor
              id="exploded"
          ...
          {BRAND_PLATES.map((piece, index) => (
            <div
              ...
              style={
                prefersReducedMotion
                  ? undefined
                  : { zIndex: BRAND_PLATES.length - index }
              }
              className={
                prefersReducedMotion
                  ? ""
                  : "md:absolute md:left-1/2 md:top-1/2 md:w-[26%] md:max-w-[230px]"
              }
            >
